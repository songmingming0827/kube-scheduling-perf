...
本地覆盖将并发固定为 `nodeLease/podPlayStage/nodePlayStage=4/1/4`；仓库 `deploy/resident/manifests/kwok-configuration.yaml` 保存可直接应用的权威 ConfigMap。不能只重新应用上游 `kwok.yaml`，否则会丢失该性能基线。
...
- Agent Scheduler 镜像：`volcanosh/vc-agent-scheduler:v1.15.1`；Scheduler Worker 为 `16`，采用事件驱动调度，不设置 Batch Scheduler 的 `schedule-period`
...
## 30. 2026-08-26 Agent Scheduler Worker 并发调整为 16

- Agent Scheduler 保持单 Deployment 副本，内部 `--scheduler-worker-count` 从 `8` 调整为 `16`；Node Worker `20`、Kubernetes API QPS/Burst `1000/1000` 和 CPU `500m/8` 均保持不变。
- 版本化 Helm values 已同步固定为 `agent_scheduler_worker_count: 16`，后续重新部署会保持该并发基线。
- 本次仅重新部署 Volcano release 并核对 Agent Scheduler 启动参数，不运行性能场景；Worker `16` 的吞吐与延迟效果需以后续 benchmark 结果判断。
